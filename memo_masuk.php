...
$search_tgl_memo = $_GET['search_tgl_memo'] ?? '';
$sort_by = $_GET['sort_by'] ?? 'tgl_terima';
$sort_order = $_GET['sort_order'] ?? 'DESC';

$search = htmlspecialchars($search);
$filter_kategori = (int)$filter_kategori;
$search_tgl = htmlspecialchars($search_tgl);
$search_tgl_memo = htmlspecialchars($search_tgl_memo);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $is_admin) {
    ...
    $tgl_terima = sanitize_data_for_bind($_POST['tgl_terima'] ?? '');
    ...
    if (!isset($_POST['csrf_token']) || !validate_csrf_token($_POST['csrf_token'])) {
        ...
        if (!$upload_error_flag) {
            ...
            if ($_POST['action_type'] == 'create') {
                $query = "INSERT INTO tb_memo_masuk (id_kategori, tgl_terima, dari, tgl_memo, no_memo, perihal, berkas) 
                          VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $koneksi->prepare($query);
                $stmt->bind_param("issssss", $id_kategori, $tgl_terima, $dari, $tgl_memo, $no_memo, $perihal, $berkas_nama);
                
                if ($stmt->execute()) {
                    header('Location: memo_masuk.php?status=sukses_tambah');
                    exit;
                } else {
                    $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
            ...
            if ($_POST['action_type'] == 'update') {
                $query = "UPDATE tb_memo_masuk SET id_kategori=?, tgl_terima=?, dari=?, tgl_memo=?, no_memo=?, perihal=?, berkas=? 
                          WHERE id_memo_masuk=?";
                $stmt = $koneksi->prepare($query);
                $stmt->bind_param("issssssi", $id_kategori, $tgl_terima, $dari, $tgl_memo, $no_memo, $perihal, $berkas_nama, $id_memo);
                
                if ($stmt->execute()) {
                    header('Location: memo_masuk.php?status=sukses_edit');
                    exit;
                } else {
                    $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
if ($action == 'delete' && $is_admin) {
    if (isset($_GET['id']) && isset($_GET['csrf_token']) && validate_csrf_token($_GET['csrf_token'])) {
        ...
        $stmt_file = $koneksi->prepare("SELECT berkas FROM tb_memo_masuk WHERE id_memo_masuk = ?");
        ...
        if ($result_file->num_rows > 0) {
            ...
            $file_to_delete = 'uploads/memo_masuk/' . $row_file['berkas']; 
            
            $stmt_delete = $koneksi->prepare("DELETE FROM tb_memo_masuk WHERE id_memo_masuk = ?");
            ...
            if ($stmt_delete->execute()) {
                ...
                header('Location: memo_masuk.php?status=sukses_hapus');
                exit;
            } else {
                $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-envelope-open-text"></i> Data Memo Masuk
        </h1>
        <?php if ($action == 'read' && $is_admin): ?>
            <a href="memo_masuk.php?action=create" class="btn btn-primary">
                <i class="fas fa-plus"></i> Tambah Memo Masuk
            </a>
        <?php endif; ?>
    </div>
    ...
    if (isset($_GET['status'])) {
        if ($_GET['status'] == 'sukses_tambah') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Memo Masuk baru berhasil ditambahkan!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_edit') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Memo Masuk berhasil diperbarui!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_hapus') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Data memo masuk berhasil dihapus!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        }
    }
    ...
    if (($action == 'create' && $is_admin) || ($action == 'edit' && $is_admin && $id_memo > 0)): 
        ...
        $data = [
            'id_memo_masuk' => 0, 'id_kategori' => '', 'tgl_terima' => '', 'dari' => '', 
            'tgl_memo' => '', 'no_memo' => '', 'perihal' => '', 'berkas' => ''
        ];
        $form_title = "Tambah Memo Masuk";
        $action_type = "create";
        
        if ($action == 'edit') {
            $stmt_edit = $koneksi->prepare("SELECT * FROM tb_memo_masuk WHERE id_memo_masuk = ?");
            ...
            if ($result_edit->num_rows == 1) {
                ...
                $form_title = "Edit Memo Masuk (ID: " . $id_memo . ")";
                $action_type = "update";
            } else {
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="memo_masuk.php?action=read&id=<?php echo $data['id_memo_masuk']; ?>" method="POST" enctype="multipart/form-data">
                
                <input type="hidden" name="action_type" value="<?php echo $action_type; ?>">
                <?php if ($action_type == 'update'): ?>
                    <input type="hidden" name="id_memo" value="<?php echo $data['id_memo_masuk']; ?>">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <input type="date" name="tgl_terima" class="form-control" value="<?php echo escape_html($data['tgl_terima']); ?>" required>
                    </div>
                    <div class="col-md-4 mb-2">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-2">
                        ...
                        <input type="text" name="dari" class="form-control" value="<?php echo escape_html($data['dari']); ?>" required>
                    </div>
                </div>
                ...
                <div class="mb-2">
                    ...
                    <?php if ($action_type == 'update' && !empty($data['berkas'])): ?>
                        <small class="form-text text-muted">
                            Berkas saat ini: <a href="uploads/memo_masuk/<?php echo rawurlencode($data['berkas']); ?>" target="_blank"><?php echo escape_html($data['berkas']); ?></a>
                        </small><br>
                ...
                <div class="mt-4">
                    ...
                    <a href="memo_masuk.php" class="btn btn-secondary">
    ...
    if ($action == 'read'): 
        ...
        $query_read = "SELECT mm.*, k.nama_kategori FROM tb_memo_masuk mm 
                         JOIN tb_kategori k ON mm.id_kategori = k.id_kategori WHERE 1=1";
        ...
        if (!empty($search)) {
            $search_param = '%' . $search . '%';
            $query_read .= " AND (mm.no_memo LIKE ? OR mm.dari LIKE ? OR mm.perihal LIKE ?)";
            $bind_types .= 'sss';
        ...
        if (!empty($search_tgl)) {
            $query_read .= " AND mm.tgl_terima = ?";
            $bind_types .= 's';
            $bind_params[] = &$search_tgl;
        }
        
        if (!empty($search_tgl_memo)) {
            $query_read .= " AND mm.tgl_memo = ?";
            $bind_types .= 's';
            $bind_params[] = &$search_tgl_memo;
        }
        
        if (!empty($filter_kategori)) {
            $query_read .= " AND mm.id_kategori = ?";
            $bind_types .= 'i';
            $bind_params[] = &$filter_kategori;
        }
        
        $allowed_sort = ['tgl_terima', 'tgl_memo', 'no_memo', 'dari', 'nama_kategori'];
        $allowed_order = ['ASC', 'DESC'];
        
        if (in_array($sort_by, $allowed_sort) && in_array($sort_order, $allowed_order)) {
            $query_read .= " ORDER BY " . $sort_by . " " . $sort_order;
        } else {
            $query_read .= " ORDER BY mm.tgl_terima DESC";
            $sort_by = 'tgl_terima';
            $sort_order = 'DESC';
        }
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="memo_masuk.php" method="GET">
                ...
                <div class="row">
                    <div class="col-md-3 mb-2">
                        ...
                        <input type="text" name="search" class="form-control" 
                               placeholder="No. Memo, Dari, atau Perihal..." 
                               value="<?php echo escape_html($search); ?>">
                    </div>
                    <div class="col-md-2 mb-2">
                        <label class="form-label"><i class="fas fa-calendar"></i> Tgl Terima</label>
                        <input type="date" name="search_tgl" class="form-control" 
                               value="<?php echo escape_html($search_tgl); ?>">
                    </div>
                    <div class="col-md-2 mb-2">
                        <label class="form-label"><i class="fas fa-calendar-alt"></i> Tgl Memo</label>
                        <input type="date" name="search_tgl_memo" class="form-control" 
                               value="<?php echo escape_html($search_tgl_memo); ?>">
                    </div>
                    <div class="col-md-3 mb-2">
                    ...
                    <div class="col-md-2 mb-2 d-flex align-items-end">
                        <div>
                            ...
                            <?php if (!empty($search) || !empty($filter_kategori) || !empty($search_tgl) || !empty($search_tgl_memo)): ?>
                                <a href="memo_masuk.php" class="btn btn-secondary" title="Reset Filter">
                ...
                <table class="table table-bordered table-hover data-table" width="100%" cellspacing="0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th width="10%">Tanggal Memo</th>
                            <th width="15%">No. Memo</th>
                            <th width="15%">Dari</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo date('d-m-Y', strtotime(escape_html($row['tgl_terima']))); ?></td>
                            ...
                            <td class="text-center">
                                <?php if (!empty($row['berkas'])): ?>
                                    <a href="uploads/memo_masuk/<?php echo rawurlencode($row['berkas']); ?>" target="_blank" 
                            ...
                            <?php if ($is_admin): ?>
                                <td class="text-center">
                                    <a href="memo_masuk.php?action=edit&id=<?php echo $row['id_memo_masuk']; ?>" 
                                       class="btn btn-warning btn-sm" title="Edit Data">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="memo_masuk.php?action=delete&id=<?php echo $row['id_memo_masuk']; ?>&csrf_token=<?php echo generate_csrf_token(); ?>" 
                        ...
                        <tr>
                            <td colspan="<?php echo $is_admin ? '9' : '8'; ?>" class="text-center">
                                Tidak ada data memo masuk yang sesuai dengan kriteria.
                            </td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
