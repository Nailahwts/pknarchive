    ...
    if (!isset($_POST['csrf_token']) || !validate_csrf_token($_POST['csrf_token'])) {
        ...
        if (!$upload_error_flag) {
            ...
            if ($_POST['action_type'] == 'create') {
                $query = "INSERT INTO tb_surat_masuk (id_kategori, tgl_terima, dari, tgl_surat, no_surat, perihal, berkas) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $koneksi->prepare($query);
                $stmt->bind_param("issssss", $id_kategori, $tgl_terima, $dari, $tgl_surat, $no_surat, $perihal, $berkas_nama);
                
                if ($stmt->execute()) {
                    header('Location: surat_masuk.php?status=sukses_tambah');
                    exit;
                } else {
                    $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
            ...
            if ($_POST['action_type'] == 'update') {
                $query = "UPDATE tb_surat_masuk SET id_kategori=?, tgl_terima=?, dari=?, tgl_surat=?, no_surat=?, perihal=?, berkas=? WHERE id_surat_masuk=?";
                $stmt = $koneksi->prepare($query);
                $stmt->bind_param("issssssi", $id_kategori, $tgl_terima, $dari, $tgl_surat, $no_surat, $perihal, $berkas_nama, $id_surat);
                
                if ($stmt->execute()) {
                    header('Location: surat_masuk.php?status=sukses_edit');
                    exit;
                } else {
                    $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
if ($action == 'delete' && $is_admin) {
    if (isset($_GET['id']) && isset($_GET['csrf_token']) && validate_csrf_token($_GET['csrf_token'])) {
        ...
        $stmt_file = $koneksi->prepare("SELECT berkas FROM tb_surat_masuk WHERE id_surat_masuk = ?");
        ...
        if ($result_file->num_rows > 0) {
            ...
            $file_to_delete = 'uploads/surat_masuk/' . $row_file['berkas']; 
            
            $stmt_delete = $koneksi->prepare("DELETE FROM tb_surat_masuk WHERE id_surat_masuk = ?");
            ...
            if ($stmt_delete->execute()) {
                ...
                header('Location: surat_masuk.php?status=sukses_hapus');
                exit;
            } else {
                $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-inbox"></i> Data Surat Masuk
        </h1>
        <?php if ($action == 'read' && $is_admin): ?>
            <a href="surat_masuk.php?action=create" class="btn btn-primary">
    ...
    if (isset($_GET['status'])) {
        if ($_GET['status'] == 'sukses_tambah') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Surat Masuk baru berhasil ditambahkan!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_edit') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Surat Masuk berhasil diperbarui!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_hapus') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Data surat masuk berhasil dihapus!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        }
    }
    ...
    if (($action == 'create' && $is_admin) || ($action == 'edit' && $is_admin && $id_surat > 0)): 
        ...
        $data = [
            'id_surat_masuk' => 0, 'id_kategori' => '', 'tgl_terima' => '', 'dari' => '', 
            'tgl_surat' => '', 'no_surat' => '', 'perihal' => '', 'berkas' => ''
        ];
        $form_title = "Tambah Surat Masuk";
        $action_type = "create";
        
        if ($action == 'edit') {
            $stmt_edit = $koneksi->prepare("SELECT * FROM tb_surat_masuk WHERE id_surat_masuk = ?");
            ...
            if ($result_edit->num_rows == 1) {
                ...
                $form_title = "Edit Surat Masuk (ID: " . $id_surat . ")";
                $action_type = "update";
            } else {
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="surat_masuk.php?action=read&id=<?php echo $data['id_surat_masuk']; ?>" method="POST" enctype="multipart/form-data">
                
                <input type="hidden" name="action_type" value="<?php echo $action_type; ?>">
                <?php if ($action_type == 'update'): ?>
                    <input type="hidden" name="id_surat" value="<?php echo $data['id_surat_masuk']; ?>">
                ...
                <div class="mb-2">
                    ...
                    <?php if ($action_type == 'update' && !empty($data['berkas'])): ?>
                        <small class="form-text text-muted">
                            Berkas saat ini: <a href="uploads/surat_masuk/<?php echo rawurlencode($data['berkas']); ?>" target="_blank"><?php echo escape_html($data['berkas']); ?></a>
                        </small><br>
    ...
    if ($action == 'read'): 
        ...
        $query_read = "SELECT sm.*, k.nama_kategori FROM tb_surat_masuk sm 
                         JOIN tb_kategori k ON sm.id_kategori = k.id_kategori WHERE 1=1";
        ...
        if (!empty($search)) {
            ...
            $search_param = '%' . $search . '%';
            $query_read .= " AND (sm.no_surat LIKE ? OR sm.dari LIKE ? OR sm.perihal LIKE ?)";
            $bind_types .= 'sss';
                ...
                <div class="row">
                    <div class="col-md-3 mb-2">
                        ...
                        <input type="text" name="search" class="form-control" 
                               placeholder="No. Surat, Dari, atau Perihal..." 
                    ...
                    <div class="col-md-2 mb-2 d-flex align-items-end">
                        <div>
                            ...
                            <?php if (!empty($search) || !empty($filter_kategori) || !empty($search_tgl) || !empty($search_tgl_surat)): ?>
                                <a href="surat_masuk.php" class="btn btn-secondary" title="Reset Filter">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <?php if (!empty($row['berkas'])): ?>
                                <a href="uploads/surat_masuk/<?php echo rawurlencode($row['berkas']); ?>" target="_blank" 
                        ...
                        <?php if ($is_admin): ?>
                            <td class="text-center">
                                <a href="surat_masuk.php?action=edit&id=<?php echo $row['id_surat_masuk']; ?>" 
                                   class="btn btn-warning btn-sm" title="Edit Data">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="surat_masuk.php?action=delete&id=<?php echo $row['id_surat_masuk']; ?>&csrf_token=<?php echo generate_csrf_token(); ?>" 
                    ...
                    <tr>
                        <td colspan="<?php echo $is_admin ? '9' : '8'; ?>" class="text-center">
                            Tidak ada data surat masuk yang sesuai dengan kriteria.
                        </td>
                    </tr>
                    <?php endif; ?>
                </tbody>
...
function build_sort_url($search, $filter_kategori, $search_tgl, $search_tgl_surat, $sort_by, $sort_order) {
    $url = 'surat_masuk.php?';
    if (!empty($search)) $url .= '&search=' . rawurlencode($search);
    if (!empty($filter_kategori)) $url .= '&filter_kategori=' . $filter_kategori;
    if (!empty($search_tgl)) $url .= '&search_tgl=' . rawurlencode($search_tgl);
