...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $is_admin) {
    ...
    $nik_karyawan = sanitize_data_for_bind($_POST['nik_karyawan'] ?? '');
    $nama_karyawan = sanitize_data_for_bind($_POST['nama_karyawan'] ?? '');
    ...
    $upload_dir = 'uploads/kontrak_staf/';

$is_post_limit_exceeded = (empty($_POST) && $_SERVER['CONTENT_LENGTH'] > 0);

    if ($is_post_limit_exceeded) {
        $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <i class="fas fa-exclamation-triangle"></i> <b>Gagal Upload!</b> Ukuran file terlalu besar melebihi batas server (post_max_size). Silakan kompres PDF Anda atau hubungi admin.
        ...
        if (strtotime($tanggal_berakhir_kontrak) < strtotime($tanggal_mulai_kontrak)) {
            ...
            if (!$upload_error_flag) {
                ...
                if ($_POST['action_type'] == 'create') {
                    $check_stmt = $koneksi->prepare("SELECT nomer_kontrak FROM tb_kontrak_staf_administrasi WHERE nomer_kontrak = ?");
                    ...
                    if ($check_result->num_rows > 0) {
                        ...
                        $query = "INSERT INTO tb_kontrak_staf_administrasi (id_kategori, nomer_kontrak, nik_karyawan, nama_karyawan, tanggal_mulai_kontrak, tanggal_berakhir_kontrak, berkas) 
                                  VALUES (?, ?, ?, ?, ?, ?, ?)";
                        $stmt = $koneksi->prepare($query);
                        $stmt->bind_param("issssss", $id_kategori, $nomer_kontrak, $nik_karyawan, $nama_karyawan, $tanggal_mulai_kontrak, $tanggal_berakhir_kontrak, $berkas_nama);
                        
                        if ($stmt->execute()) {
                            header('Location: kontrak_staf.php?status=sukses_tambah');
                            exit;
                        } else {
                            $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                ...
                if ($_POST['action_type'] == 'update') {
                    $query = "UPDATE tb_kontrak_staf_administrasi SET id_kategori=?, nomer_kontrak=?, nik_karyawan=?, nama_karyawan=?, tanggal_mulai_kontrak=?, tanggal_berakhir_kontrak=?, berkas=? 
                              WHERE id_kontrak_staf=?";
                    $stmt = $koneksi->prepare($query);
                    $stmt->bind_param("issssssi", $id_kategori, $nomer_kontrak, $nik_karyawan, $nama_karyawan, $tanggal_mulai_kontrak, $tanggal_berakhir_kontrak, $berkas_nama, $id_kontrak);
                    
                    if ($stmt->execute()) {
                        header('Location: kontrak_staf.php?status=sukses_edit');
                        exit;
                    } else {
                        $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
if ($action == 'delete' && $is_admin) {
    if (isset($_GET['id']) && isset($_GET['csrf_token']) && validate_csrf_token($_GET['csrf_token'])) {
        ...
        $stmt_file = $koneksi->prepare("SELECT berkas FROM tb_kontrak_staf_administrasi WHERE id_kontrak_staf = ?");
        ...
        if ($result_file->num_rows > 0) {
            ...
            $file_to_delete = 'uploads/kontrak_staf/' . $row_file['berkas']; 
            
            $stmt_delete = $koneksi->prepare("DELETE FROM tb_kontrak_staf_administrasi WHERE id_kontrak_staf = ?");
            ...
            if ($stmt_delete->execute()) {
                ...
                header('Location: kontrak_staf.php?status=sukses_hapus');
                exit;
            } else {
                $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-user-tie"></i> Data Kontrak Staf Administrasi
        </h1>
        <?php if ($action == 'read' && $is_admin): ?>
            <a href="kontrak_staf.php?action=create" class="btn btn-primary">
                <i class="fas fa-plus"></i> Tambah Kontrak Staf
            </a>
        <?php endif; ?>
    </div>
    ...
    if (isset($_GET['status'])) {
        if ($_GET['status'] == 'sukses_tambah') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Kontrak Staf Administrasi baru berhasil ditambahkan!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_edit') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Kontrak Staf Administrasi berhasil diperbarui!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        } elseif ($_GET['status'] == 'sukses_hapus') {
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                            <i class="fas fa-check-circle"></i> Data kontrak staf administrasi berhasil dihapus!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        }
    }
    ...
    if (($action == 'create' && $is_admin) || ($action == 'edit' && $is_admin && $id_kontrak > 0)): 
        ...
        $data = [
            'id_kontrak_staf' => 0, 'id_kategori' => '', 'nomer_kontrak' => '', 'nik_karyawan' => '',
            'nama_karyawan' => '', 'tanggal_mulai_kontrak' => '', 'tanggal_berakhir_kontrak' => '', 'berkas' => ''
        ];
        $form_title = "Tambah Kontrak Staf Administrasi";
        $action_type = "create";
        
        if ($action == 'edit') {
            $stmt_edit = $koneksi->prepare("SELECT * FROM tb_kontrak_staf_administrasi WHERE id_kontrak_staf = ?");
            ...
            if ($result_edit->num_rows == 1) {
                ...
                $form_title = "Edit Kontrak Staf Administrasi (ID: " . $id_kontrak . ")";
                $action_type = "update";
            } else {
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="kontrak_staf.php?action=read&id=<?php echo $data['id_kontrak_staf']; ?>" method="POST" enctype="multipart/form-data">
                
                <input type="hidden" name="action_type" value="<?php echo $action_type; ?>">
                <?php if ($action_type == 'update'): ?>
                    <input type="hidden" name="id_kontrak" value="<?php echo $data['id_kontrak_staf']; ?>">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <?php if ($action_type == 'create'): ?>
                            <small class="text-muted">Contoh: SA/2025/001</small>
                        <?php endif; ?>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="nik_karyawan" class="form-control" 
                               value="<?php echo escape_html($data['nik_karyawan']); ?>" 
                               maxlength="50" required>
                        <small class="text-muted">NIK sesuai KTP</small>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" name="nama_karyawan" class="form-control" value="<?php echo escape_html($data['nama_karyawan']); ?>" required>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <?php if ($action_type == 'update' && !empty($data['berkas'])): ?>
                        <small class="form-text text-muted">
                            Berkas saat ini: <a href="uploads/kontrak_staf/<?php echo rawurlencode($data['berkas']); ?>" target="_blank"><?php echo escape_html($data['berkas']); ?></a>
                        </small><br>
                ...
                <div class="mt-4">
                    ...
                    <a href="kontrak_staf.php" class="btn btn-secondary">
    ...
    if ($action == 'read'): 
        ...
        $query_read = "SELECT ksa.*, k.nama_kategori FROM tb_kontrak_staf_administrasi ksa 
                         JOIN tb_kategori k ON ksa.id_kategori = k.id_kategori WHERE 1=1";
        ...
        if (!empty($search)) {
            $search_param = '%' . $search . '%';
            $query_read .= " AND (ksa.nomer_kontrak LIKE ? OR ksa.nik_karyawan LIKE ? OR ksa.nama_karyawan LIKE ?)";
            $bind_types .= 'sss';
        ...
        if (!empty($search_tgl_mulai)) {
            $query_read .= " AND ksa.tanggal_mulai_kontrak = ?";
            $bind_types .= 's';
            $bind_params[] = &$search_tgl_mulai;
        }
        
        if (!empty($search_tgl_akhir)) {
            $query_read .= " AND ksa.tanggal_berakhir_kontrak = ?";
            $bind_types .= 's';
            $bind_params[] = &$search_tgl_akhir;
        }

        if (!empty($filter_kategori)) {
            $query_read .= " AND ksa.id_kategori = ?";
            $bind_types .= 'i';
        ...
        if ($filter_status == 'aktif') {
            $query_read .= " AND ksa.tanggal_berakhir_kontrak > '$warning_date'";
        } elseif ($filter_status == 'akan_berakhir') {
            $query_read .= " AND ksa.tanggal_berakhir_kontrak BETWEEN '$today' AND '$warning_date'";
        } elseif ($filter_status == 'kadaluarsa') {
            $query_read .= " AND ksa.tanggal_berakhir_kontrak < '$today'";
        }
        
        $allowed_sort = ['tanggal_mulai_kontrak', 'tanggal_berakhir_kontrak', 'nomer_kontrak', 'nik_karyawan', 'nama_karyawan', 'nama_kategori'];
        $allowed_order = ['ASC', 'DESC'];
        
        if (in_array($sort_by, $allowed_sort) && in_array($sort_order, $allowed_order)) {
            $query_read .= " ORDER BY " . $sort_by . " " . $sort_order;
        } else {
            $query_read .= " ORDER BY ksa.tanggal_berakhir_kontrak ASC";
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="kontrak_staf.php" method="GET">
                ...
                <div class="row">
                    <div class="col-md-5 mb-3">
                        ...
                        <input type="text" name="search" class="form-control" 
                               placeholder="No. Kontrak, NIK, atau Nama..." 
                    ...
                    <div class="col-md-1 mb-3 d-flex align-items-end">
                        <div>
                            ...
                            <?php if (!empty($search) || !empty($filter_kategori) || !empty($search_tgl_mulai) || !empty($search_tgl_akhir) || !empty($filter_status)): ?>
                                <a href="kontrak_staf.php" class="btn btn-secondary" title="Reset Filter">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                Daftar Kontrak Staf Administrasi
                <?php if (!$is_admin): ?>
                ...
                <table class="table table-bordered table-hover data-table" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo escape_html($row['nik_karyawan']); ?></td>
                            <td><?php echo escape_html($row['nama_karyawan']); ?></td>
                            ...
                            <td class="text-center">
                                <?php if (!empty($row['berkas'])): ?>
                                    <a href="uploads/kontrak_staf/<?php echo rawurlencode($row['berkas']); ?>" target="_blank" 
                            ...
                            <?php if ($is_admin): ?>
                                <td class="text-center">
                                    <a href="kontrak_staf.php?action=edit&id=<?php echo $row['id_kontrak_staf']; ?>" 
                                       class="btn btn-warning btn-sm" title="Edit Data">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="kontrak_staf.php?action=delete&id=<?php echo $row['id_kontrak_staf']; ?>&csrf_token=<?php echo generate_csrf_token(); ?>" 
                        ...
                        ?>
                        <tr>
                            <td colspan="<?php echo $is_admin ? '11' : '10'; ?>" class="text-center">
                                Tidak ada data kontrak staf administrasi yang sesuai dengan kriteria.
                            </td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
